 ...
 *
 * Everything rendered through RecordingHealthStripView is handed a snapshot
 * this file diagnosed at the frozen NOW, so a fixed date is exactly right
 * there. The connected tests are not: they go through the hook, which
 * diagnoses whatever /ingest-status answers against Date.now(). Ages built
 * from the frozen NOW therefore grow by a day for every day that passes, and
 * a fixture meant to read "last chunk 12s ago" started reading "no chunk for
 * 15h" - green on the morning it was written, amber by that evening, and a
 * failing job on master ever after.
 */
 ...
 *
 * Read by the hook, which diagnoses against the real clock - so its ages are
 * built from the real clock too. See isoFromRealNow.
 */
